...
// Call all functions
 const Appl = {
   init: function(){
     ...
     //1st = get randomly 4 letters from middleFilterLetters
     getFourLettersFromAll : function(){
       while ( arrFourLettersFromAll.length < 4) {
         // shuffle letters of array
         const allRandomLetter = middleFilterLetters[Math.floor(Math.random() * middleFilterLetters.length)]
       ...
       shuffleArrayConst = shuffleArray(arrFourLettersFromAll)
        ...
        for (i = 0; i < shuffleArrayConst.length ; i++){
          ...
          function shuffleArrayAgain(shuffleArrayConst){
            ...
            shuffleMain = alphabet.find(i => i.middle == shuffleArrayConstAgain[0]);
